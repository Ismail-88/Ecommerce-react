...
const ProductImageGallery = ({
  ...
  return (
    <div className="lg:sticky lg:top-24">
      {/* Main Image */}
      <div
        className="relative bg-surface border border-border rounded-lg overflow-hidden group/main"
        onMouseEnter={() => setZoom(true)}
        onMouseLeave={() => setZoom(false)}
      >
        {!imageLoaded && (
          <div className="absolute inset-0 animate-pulse bg-surface-strong" aria-hidden />
        )}
        <img
          src={activeImage}
          alt={productTitle}
          onLoad={() => setImageLoaded(true)}
          className={`p-6 sm:p-8 w-full h-[340px] lg:h-[440px] object-contain rounded-lg transition-transform duration-300 ${
        ...
        <div className="absolute top-4 right-4 flex gap-2">
          <button
            ...
            className={`p-2.5 rounded-lg border backdrop-blur transition-all active:scale-90 ${
              isWishlisted
          ...
          <div className="relative">
            <button
              ...
              className="p-2.5 rounded-lg border border-border bg-surface-alt text-text-secondary hover:text-foreground hover:border-border-strong transition-all active:scale-90"
            >
              <Share2 size={18} aria-hidden />
            </button>

            {showShareMenu && (
              <div className="absolute right-0 mt-2 rounded-lg border border-border bg-surface shadow-overlay p-1.5 w-44 z-20 animate-scale-in">
                {shareOptions.map(({ key, label }) => (
                  <button
                    key={key}
                    onClick={() => handleShare(key)}
                    className="w-full text-left px-3.5 py-2 hover:bg-surface-hover rounded-md text-sm transition capitalize"
                  >
                    {label}
                  </button>
                ))}
        ...
        {/* Discount Badge */}
        {discount > 0 && (
          <div className="absolute top-4 left-4 inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full bg-brand-600 text-white text-xs font-bold shadow-sm">
      ...
      {images.length > 1 && (
        <div className="flex gap-2.5 overflow-x-auto mt-3 pb-1.5 [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
          {images.map((url, index) => (
            <button
              ...
              className={`flex-shrink-0 bg-surface border-2 rounded-md overflow-hidden transition-all ${
                selectedImage === index
            ...
            >
              <img
                ...
                className="w-16 h-16 sm:w-[72px] sm:h-[72px] object-contain p-1.5"
              />
            </button>
          ))}
        </div>
